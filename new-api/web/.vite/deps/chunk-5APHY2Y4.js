...
function addBorderSegments(g) {
  function dfs3(v) {
    ...
    if (Object.prototype.hasOwnProperty.call(node, 'minRank')) {
      ...
      node.borderRight = [];
      for (
        var rank2 = node.minRank, maxRank2 = node.maxRank + 1;
        rank2 < maxRank2;
        ++rank2
      ) {
...
function buildLayerGraph(g, rank2, relationship) {
  ...
  forEach_default(g.nodes(), function (v) {
    ...
    if (
      node.rank === rank2 ||
      (node.minRank <= rank2 && rank2 <= node.maxRank)
    ) {
      result.setNode(v);
...
function order(g) {
  var maxRank2 = maxRank(g),
    downLayerGraphs = buildLayerGraphs(
      g,
      range_default(1, maxRank2 + 1),
      'inEdges',
    ),
    upLayerGraphs = buildLayerGraphs(
      g,
      range_default(maxRank2 - 1, -1, -1),
      'outEdges',
    );
...
function hasConflict(conflicts, v, w) {
  ...
  return (
    !!conflicts[v] && Object.prototype.hasOwnProperty.call(conflicts[v], w)
  );
}
...
function alignCoordinates(xss, alignTo) {
  ...
  forEach_default(['u', 'd'], function (vert) {
    forEach_default(['l', 'r'], function (horiz) {
      ...
      delta =
        horiz === 'l'
          ? alignToMin - min_default(xsVals)
          : alignToMax - max_default(xsVals);
      if (delta) {
...
function positionX(g) {
  ...
  forEach_default(['u', 'd'], function (vert) {
    adjustedLayering =
      vert === 'u' ? layering : values_default(layering).reverse();
    forEach_default(['l', 'r'], function (horiz) {
      if (horiz === 'r') {
...
function buildLayoutGraph(inputGraph) {
  ...
  forEach_default(inputGraph.nodes(), function (v) {
    ...
    g.setNode(
      v,
      defaults_default(selectNumberAttrs(node, nodeNumAttrs), nodeDefaults),
    );
...
export { layout };
//# sourceMappingURL=chunk-5APHY2Y4.js.map
