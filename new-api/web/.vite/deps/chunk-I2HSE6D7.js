import { createLabel_default } from './chunk-P32SJDBU.js';
...
import { getSubGraphTitleMargins } from './chunk-GCN5RWLQ.js';
import { createText } from './chunk-KS234T37.js';
import { utils_default } from './chunk-IPOIRDR2.js';
import { getConfig2, getEffectiveHtmlLabels } from './chunk-LQLXUK6X.js';
import {
  __name,
  basis_default,
  bumpX,
...
var addEdgeMarkers = __name(
  (svgPath, edge, url, id, diagramType, strokeColor) => {
...
var addEdgeMarker = __name(
  (svgPath, position, arrowType, url, id, diagramType, strokeColor) => {
    var _a;
    ...
    if (strokeColor && strokeColor.trim() !== '') {
      ...
      if (!document.getElementById(coloredMarkerId)) {
        ...
        if (originalMarker) {
          ...
          (_a = originalMarker.parentNode) == null
            ? void 0
            : _a.appendChild(coloredMarker);
        }
      }
...
var generateDashArray = __name((len, oValueS, oValueE) => {
  ...
  const middlePattern = Array(numberOfPairs)
...
var insertEdge = __name(function (
  ...
  if (edge.toCluster) {
    ...
    points = cutPathAtIntersect(
      edge.points,
      clusterDb.get(edge.toCluster).node,
    );
  ...
  if (edge.fromCluster) {
    ...
    points = cutPathAtIntersect(
      points.reverse(),
      clusterDb.get(edge.fromCluster).node,
    ).reverse();
    pointsHasChanged = true;
  }
  ...
  if (edge.look === 'handDrawn') {
    ...
    svgPath = select_default(svgPathNode)
      ...
      .attr(
        'style',
        edgeStyles
          ? edgeStyles.reduce((acc, style) => acc + ';' + style, '')
          : '',
      );
    ...
    strokeColor =
      (_a = pathStyle.match(/stroke:([^;]+)/)) == null ? void 0 : _a[1];
    animatedEdge =
      edge.animate === true ||
      !!edge.animation ||
      stylesFromClasses.includes('animation');
...
//# sourceMappingURL=chunk-I2HSE6D7.js.map
