...
function max(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    for (let value of values) {
      if ((value = valueof(value, ++index5, values)) != null && (max8 < value || max8 === void 0 && value >= value)) {
        max8 = value;
      }
    }
  }
...
function min(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index5 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index5, values)) != null && (min7 > value || min7 === void 0 && value >= value)) {
        min7 = value;
      }
    }
  }
...
function shuffler(random) {
  return function shuffle2(array7, i0 = 0, i1 = array7.length) {
    let m = i1 - (i0 = +i0);
    while (m) {
      const i = random() * m-- | 0, t = array7[m + i0];
...
function selectAll_default(select) {
  ...
  for (var groups5 = this._groups, m = groups5.length, subgroups = [], parents = [], j = 0; j < m; ++j) {
...
function locale_default(locale3) {
  ...
  function newFormat(specifier, options) {
    ...
    if (zero6 || fill === "0" && align === "=") zero6 = true, fill = "0", align = "=";
    ...
    function format3(value) {
      ...
      if (comma && zero6) value = group5(padding + value, padding.length ? width - valueSuffix.length : Infinity), padding = "";
      switch (align) {
        case "<":
...
function ticks2(start2, stop, count6) {
  ...
  const reverse5 = stop < start2, [i1, i2, inc] = reverse5 ? tickSpec(stop, start2, count6) : tickSpec(start2, stop, count6);
  if (!(i2 >= i1)) return [];
...
function tickStep2(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  const reverse5 = stop < start2, inc = reverse5 ? tickIncrement2(stop, start2, count6) : tickIncrement2(start2, stop, count6);
...
function transformer() {
  ...
  function scale2(x3) {
    return x3 == null || isNaN(x3 = +x3) ? unknown : (output || (output = piecewise2(domain.map(transform2), range5, interpolate)))(transform2(clamp(x3)));
  }
  scale2.invert = function(y3) {
    return clamp(untransform((input || (input = piecewise2(range5, domain.map(transform2), number_default)))(y3)));
  };
  scale2.domain = function(_) {
...
function tickFormat(start2, stop, count6, specifier) {
  var step = tickStep2(start2, stop, count6), precision;
...
function tickSpec2(start2, stop, count6) {
  ...
  if (i2 < i1 && 0.5 <= count6 && count6 < 2) return tickSpec2(start2, stop, count6 * 2);
  return [i1, i2, inc];
}
...
function tickStep3(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  const reverse5 = stop < start2, inc = reverse5 ? tickIncrement3(stop, start2, count6) : tickIncrement3(start2, stop, count6);
  return (reverse5 ? -1 : 1) * (inc < 0 ? 1 / -inc : inc);
}
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function tickInterval(start2, stop, count6) {
    ...
    if (i === tickIntervals.length) return year.every(tickStep3(start2 / durationYear, stop / durationYear, count6));
    if (i === 0) return millisecond.every(Math.max(tickStep3(start2, stop, count6), 1));
...
function calendar(ticks5, tickInterval, year, month, week, day, hour, minute, second2, format3) {
  ...
  scale2.copy = function() {
    return copy(scale2, calendar(ticks5, tickInterval, year, month, week, day, hour, minute, second2, format3));
  };
  return scale2;
}
function time() {
...
function cornerTangents(x02, y0, x12, y1, r1, rc, cw) {
  var x01 = x02 - x12, y01 = y0 - y1, lo = (cw ? rc : -rc) / sqrt2(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x02 + ox, y11 = y0 + oy, x10 = x12 + ox, y10 = y1 + oy, x00 = (x11 + x10) / 2, y00 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D3 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt2(max4(0, r * r * d2 - D3 * D3)), cx0 = (D3 * dy - dx * d) / d2, cy0 = (-D3 * dx - dy * d) / d2, cx1 = (D3 * dy + dx * d) / d2, cy1 = (-D3 * dx + dy * d) / d2, dx0 = cx0 - x00, dy0 = cy0 - y00, dx1 = cx1 - x00, dy1 = cy1 - y00;
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon3)) context.moveTo(0, 0);
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon3 && (padRadius ? +padRadius.apply(this, arguments) : sqrt2(r0 * r0 + r1 * r1)), rc = min4(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
      if (rp > epsilon3) {
...
function pie_default() {
  var value = identity_default3, sortValues = descending_default, sort5 = null, startAngle = constant_default4(0), endAngle = constant_default4(tau), padAngle = constant_default4(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum7 = 0, index5 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau, Math.max(-tau, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
    ...
    else if (sort5 != null) index5.sort(function(i2, j2) {
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum7.add(atan22(k2 * sign3 * sin3(absDelta), cosPhi0 * cosPhi1 + k2 * cos3(absDelta)));
      ...
      if (antimeridian ^ lambda0 >= lambda ^ lambda1 >= lambda) {
        ...
        var phiArc = (antimeridian ^ delta >= 0 ? -1 : 1) * asin2(intersection5[2]);
  ...
  return (angle < -epsilon7 || angle < epsilon7 && sum7 < -epsilon22) ^ winding & 1;
}
