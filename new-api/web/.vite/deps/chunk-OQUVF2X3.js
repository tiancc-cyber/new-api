...
function max(values, valueof) {
  ...
  if (valueof === void 0) {
    for (const value of values) {
      if (
        value != null &&
        (max8 < value || (max8 === void 0 && value >= value))
      ) {
        max8 = value;
      }
    }
    ...
    for (let value of values) {
      if (
        (value = valueof(value, ++index5, values)) != null &&
        (max8 < value || (max8 === void 0 && value >= value))
      ) {
        max8 = value;
      }
...
function min(values, valueof) {
  ...
  if (valueof === void 0) {
    for (const value of values) {
      if (
        value != null &&
        (min7 > value || (min7 === void 0 && value >= value))
      ) {
        min7 = value;
      }
    }
...
function blurf(radius) {
  ...
  return (T, S, start2, stop, step) => {
    ...
    for (let i = start2, j = stop; i <= j; i += step) {
      ...
      T[i] =
        (sum7 + t * (S[Math.max(start2, i - s1)] + S[Math.min(stop, i + s1)])) /
        w;
...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values =
      ...
      range5 = scale2.range(),
      range0 = +range5[0] + offset,
      range1 = +range5[range5.length - 1] + offset,
...
function select_default(select) {
  ...
  for (
    ...
    for (
      ...
      if (
        (node = group5[i]) &&
        (subnode = select.call(node, node.__data__, i, group5))
      ) {
...
function data_default(value, key) {
  ...
  var bind = key ? bindKey : bindIndex,
    ...
    groups5 = this._groups;
  if (typeof value !== 'function') value = constant_default(value);
  for (
    var m = groups5.length,
      update = new Array(m),
      enter = new Array(m),
      exit = new Array(m),
      j = 0;
...
function order_default() {
  for (var groups5 = this._groups, j = -1, m = groups5.length; ++j < m; ) {
    for (
      var group5 = groups5[j], i = group5.length - 1, next = group5[i], node;
      --i >= 0;
...
function each_default(callback) {
  for (var groups5 = this._groups, j = 0, m = groups5.length; j < m; ++j) {
    for (var group5 = groups5[j], i = 0, n = group5.length, node; i < n; ++i) {
      if ((node = group5[i])) callback.call(node, node.__data__, i, group5);
    }
  }
  return this;
}
...
function lrgb2rgb(x3) {
  return (
    255 * (x3 <= 31308e-7 ? 12.92 * x3 : 1.055 * Math.pow(x3, 1 / 2.4) - 0.055)
  );
}
function rgb2lrgb(x3) {
  return (x3 /= 255) <= 0.04045
    ? x3 / 12.92
    : Math.pow((x3 + 0.055) / 1.055, 2.4);
}
...
function hcl(h, c3, l, opacity) {
  return arguments.length === 1
    ? hclConvert(h)
    : new Hcl(h, c3, l, opacity == null ? 1 : opacity);
}
function Hcl(h, c3, l, opacity) {
  this.h = +h;
  this.c = +c3;
    ...
    : i > n
      ...
      : i > 0
        ...
        : '0.' +
          ...
          formatDecimalParts(x3, Math.max(0, p + i - 1))[0];
}
...
function locale_default(locale3) {
  ...
  function newFormat(specifier, options) {
    ...
    function format3(value) {
        ...
        if (maybeSuffix) {
          ...
          while (++i < n) {
            if (((c3 = value.charCodeAt(i)), 48 > c3 || c3 > 57)) {
              valueSuffix =
                (c3 === 46 ? decimal + value.slice(i + 1) : value.slice(i)) +
                valueSuffix;
      ...
      if (comma && !zero6) value = group5(value, Infinity);
...
function count2(node) {
  ...
  if (!i) sum7 = 1;
  else while (--i >= 0) sum7 += children2[i].value;
  node.value = sum7;
}
...
function sum_default(value) {
  return this.eachAfter(function (node) {
    var sum7 = +value(node.data) || 0,
      children2 = node.children,
      i = children2 && children2.length;
    while (--i >= 0) sum7 += children2[i].value;
    node.value = sum7;
  });
}
...
function treemap_default() {
  ...
  treemap.padding = function (x3) {
    return arguments.length
      ? treemap.paddingInner(x3).paddingOuter(x3)
      : treemap.paddingInner();
  };
  treemap.paddingInner = function (x3) {
    return arguments.length
      ? ((paddingInner =
          typeof x3 === 'function' ? x3 : constant_default3(+x3)),
        treemap)
      : paddingInner;
  };
  ...
  treemap.paddingTop = function (x3) {
    return arguments.length
      ? ((paddingTop = typeof x3 === 'function' ? x3 : constant_default3(+x3)),
        treemap)
      : paddingTop;
  };
  ...
  treemap.paddingLeft = function (x3) {
    return arguments.length
      ? ((paddingLeft = typeof x3 === 'function' ? x3 : constant_default3(+x3)),
        treemap)
      : paddingLeft;
  };
  return treemap;
      ...
      while (++j < m) {
        ...
        if (row.dice)
          ...
          slice_default(
            ...
            x02,
            y0,
            value ? (x02 += ((x12 - x02) * row.value) / value) : x12,
            y1,
          );
        value -= row.value;
      }
...
function tickSpec(start2, stop, count6) {
  const step = (stop - start2) / Math.max(0, count6),
...
function ticks2(start2, stop, count6) {
  ...
  const reverse5 = stop < start2,
    [i1, i2, inc] = reverse5
      ? tickSpec(stop, start2, count6)
      : tickSpec(start2, stop, count6);
  if (!(i2 >= i1)) return [];
  const n = i2 - i1 + 1,
    ticks5 = new Array(n);
  if (reverse5) {
    if (inc < 0) for (let i = 0; i < n; ++i) ticks5[i] = (i2 - i) / -inc;
    else for (let i = 0; i < n; ++i) ticks5[i] = (i2 - i) * inc;
  } else {
    if (inc < 0) for (let i = 0; i < n; ++i) ticks5[i] = (i1 + i) / -inc;
    else for (let i = 0; i < n; ++i) ticks5[i] = (i1 + i) * inc;
  }
  return ticks5;
}
...
function tickStep2(start2, stop, count6) {
  ...
  const reverse5 = stop < start2,
    inc = reverse5
      ? tickIncrement2(stop, start2, count6)
      : tickIncrement2(start2, stop, count6);
...
function polymap(domain, range5, interpolate) {
  ...
  return function (x3) {
    var i2 = bisect_default2(domain, x3, 1, j) - 1;
    return r[i2](d[i2](x3));
  };
}
...
function transformer() {
  ...
  function scale2(x3) {
    return x3 == null || isNaN((x3 = +x3))
      ...
      : (
          output ||
          (output = piecewise2(domain.map(transform2), range5, interpolate))
        )(transform2(clamp(x3)));
  }
  scale2.invert = function (y3) {
    return clamp(
      untransform(
        (
          input ||
          (input = piecewise2(range5, domain.map(transform2), number_default))
        )(y3),
      ),
    );
  };
  scale2.domain = function (_) {
    return arguments.length
      ? ((domain = Array.from(_, number4)), rescale())
      : domain.slice();
  };
  scale2.range = function (_) {
    return arguments.length
      ? ((range5 = Array.from(_)), rescale())
      : range5.slice();
  };
  scale2.rangeRound = function (_) {
  ...
  scale2.clamp = function (_) {
    return arguments.length
      ? ((clamp = _ ? true : identity4), rescale())
      : clamp !== identity4;
  };
...
function tickFormat(start2, stop, count6, specifier) {
  var step = tickStep2(start2, stop, count6),
    precision;
...
function linearish(scale2) {
  ...
  scale2.ticks = function (count6) {
    var d = domain();
    return ticks2(d[0], d[d.length - 1], count6 == null ? 10 : count6);
  };
  ...
  scale2.nice = function (count6) {
    ...
    while (maxIter-- > 0) {
      step = tickIncrement2(start2, stop, count6);
      if (step === prestep) {
        d[i0] = start2;
        d[i1] = stop;
        return domain(d);
...
function timeInterval(floori, offseti, count6, field) {
  ...
  interval2.range = (start2, stop, step) => {
    ...
    let previous;
    do
      range5.push((previous = /* @__PURE__ */ new Date(+start2))),
        offseti(start2, step),
        floori(start2);
...
function bisector3(f) {
  ...
  if (f.length !== 2) {
    compare1 = ascending4;
    compare2 = (d, x3) => ascending4(f(d), x3);
    delta = (d, x3) => f(d) - x3;
  } else {
...
var bisectCenter3 = bisector3(number5).center;
...
function tickSpec2(start2, stop, count6) {
  const step = (stop - start2) / Math.max(0, count6),
    power = Math.floor(Math.log10(step)),
    error = step / Math.pow(10, power),
    factor = error >= e103 ? 10 : error >= e53 ? 5 : error >= e23 ? 2 : 1;
  let i1, i2, inc;
  if (power < 0) {
    inc = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc);
...
function tickStep3(start2, stop, count6) {
  ...
  const reverse5 = stop < start2,
    inc = reverse5
      ? tickIncrement3(stop, start2, count6)
      : tickIncrement3(start2, stop, count6);
  return (reverse5 ? -1 : 1) * (inc < 0 ? 1 / -inc : inc);
}
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function ticks5(start2, stop, count6) {
    ...
    const interval2 =
      count6 && typeof count6.range === 'function'
        ? count6
        : tickInterval(start2, stop, count6);
  ...
  function tickInterval(start2, stop, count6) {
    ...
    if (i === tickIntervals.length)
      return year.every(
        tickStep3(start2 / durationYear, stop / durationYear, count6),
      );
    if (i === 0)
      return millisecond.every(Math.max(tickStep3(start2, stop, count6), 1));
    const [t, step] =
      tickIntervals[
...
var Path = class {
  ...
  moveTo(x3, y3) {
    this
      ._append`M${(this._x0 = this._x1 = +x3)},${(this._y0 = this._y1 = +y3)}`;
  }
  closePath() {
    if (this._x1 !== null) {
  ...
  bezierCurveTo(x12, y1, x22, y22, x3, y3) {
    this
      ._append`C${+x12},${+y1},${+x22},${+y22},${(this._x1 = +x3)},${(this._y1 = +y3)}`;
  }
  ...
  arc(x3, y3, r, a0, a1, ccw) {
    ...
    if (this._x1 === null) {
      ...
      Math.abs(this._x1 - x02) > epsilon4 ||
      Math.abs(this._y1 - y0) > epsilon4
    ) {
...
function cornerTangents(x02, y0, x12, y1, r1, rc, cw) {
  var x01 = x02 - x12,
    ...
    r = r1 - rc,
    D3 = x11 * y10 - x10 * y11,
    d = (dy < 0 ? -1 : 1) * sqrt2(max4(0, r * r * d2 - D3 * D3)),
    cx0 = (D3 * dy - dx * d) / d2,
    cy0 = (-D3 * dx - dy * d) / d2,
    cx1 = (D3 * dy + dx * d) / d2,
    cy1 = (-D3 * dx + dy * d) / d2,
...
function arc_default() {
      ...
      var a01 = a0,
        ...
        rp =
          ...
          (padRadius
            ...
            : sqrt2(r0 * r0 + r1 * r1)),
        rc = min4(abs(r1 - r0) / 2, +cornerRadius.apply(this, arguments)),
        rc0 = rc,
        rc1 = rc,
        t03,
        t13;
            ...
            var ax = x01 - oc[0],
              ...
              kc =
                ...
                sin(
                  acos(
                    (ax * bx + ay * by) /
                      (sqrt2(ax * ax + ay * ay) * sqrt2(bx * bx + by * by)),
                  ) / 2,
                ),
      ...
      if (!(da1 > epsilon3)) context.moveTo(x01, y01);
        ...
        if (rc1 < rc)
          context.arc(
            ...
            t03.cy,
            rc1,
            atan2(t03.y01, t03.x01),
            atan2(t13.y01, t13.x01),
            !cw,
          );
        else {
          context.arc(
  ...
  arc.innerRadius = function (_) {
    return arguments.length
      ? ((innerRadius = typeof _ === 'function' ? _ : constant_default4(+_)),
        arc)
      : innerRadius;
  };
  arc.outerRadius = function (_) {
    return arguments.length
      ? ((outerRadius = typeof _ === 'function' ? _ : constant_default4(+_)),
        arc)
      : outerRadius;
  };
  arc.cornerRadius = function (_) {
    return arguments.length
      ? ((cornerRadius = typeof _ === 'function' ? _ : constant_default4(+_)),
        arc)
      : cornerRadius;
  };
  ...
  arc.startAngle = function (_) {
    return arguments.length
      ? ((startAngle = typeof _ === 'function' ? _ : constant_default4(+_)),
        arc)
      : startAngle;
  };
...
function line_default(x3, y3) {
  var defined = constant_default4(true),
    ...
    path2 = withPath(line);
  x3 =
    typeof x3 === 'function' ? x3 : x3 === void 0 ? x : constant_default4(x3);
  y3 =
    typeof y3 === 'function' ? y3 : y3 === void 0 ? y : constant_default4(y3);
  function line(data) {
    var i,
  ...
  point: function (x3, y3) {
    ...
    switch (this._point) {
      ...
      case 2:
        ...
        this._context.moveTo(
          (this._x0 + 4 * this._x1 + x3) / 6,
          (this._y0 + 4 * this._y1 + y3) / 6,
        );
        break;
      default:
        point2(this, x3, y3);
  ...
  point: function (x3, y3) {
    ...
    switch (this._point) {
      ...
      case 2:
        ...
        this._line
          ? this._context.lineTo(x02, y0)
          : this._context.moveTo(x02, y0);
        break;
      case 3:
        this._point = 4;
      default:
...
Natural.prototype = {
  ...
  lineEnd: function () {
    ...
    if (n) {
      this._line
        ? this._context.lineTo(x3[0], y3[0])
        : this._context.moveTo(x3[0], y3[0]);
      if (n === 2) {
...
var ENDING = 5;
var ENDED = 6;
function schedule_default(node, name, id2, index5, group5, timing) {
...
function selectAll_default3(select) {
  ...
  for (
    ...
    for (var group5 = groups5[j], n = group5.length, node, i = 0; i < n; ++i) {
      if ((node = group5[i])) {
        for (
          var children2 = select.call(node, node.__data__, i, group5),
            child,
...
var { abs: abs2, max: max5, min: min5 } = Math;
function number1(e) {
  return [+e[0], +e[1]];
}
function number22(e) {
...
function bisector4(f) {
  ...
  if (f.length !== 2) {
    compare1 = ascending5;
    compare2 = (d, x3) => ascending5(f(d), x3);
    delta = (d, x3) => f(d) - x3;
  } else {
...
var bisectCenter4 = bisector4(number7).center;
...
function blurf4(radius) {
  const radius0 = Math.floor(radius);
  if (radius0 === radius) return bluri4(radius);
  const t = radius - radius0;
  const w = 2 * radius + 1;
  return (T, S, start2, stop, step) => {
    if (!((stop -= step) >= start2)) return;
        ...
        (milliseconds2
          ...
          : seconds2
            ? 'T' +
              ...
              'Z'
            : minutes || hours
              ? 'T' + pad3(hours, 2) + ':' + pad3(minutes, 2) + 'Z'
              : '');
}
function dsv_default2(delimiter) {
  ...
  function parse(text, f) {
    var convert,
      ...
      rows = parseRows(text, function (row, i) {
        if (convert) return convert(row, i - 1);
        (columns = row),
          (convert = f ? customConverter2(row, f) : objectConverter2(row));
      });
    rows.columns = columns || [];
    return rows;
  }
  function parseRows(text, f) {
    var rows = [],
      ...
      eof = N <= 0,
      eol = false;
    if (text.charCodeAt(N - 1) === NEWLINE2) --N;
    if (text.charCodeAt(N - 1) === RETURN2) --N;
    function token() {
      if (eof) return EOF2;
      ...
      if (text.charCodeAt(j) === QUOTE2) {
        while (
          (I++ < N && text.charCodeAt(I) !== QUOTE2) ||
          text.charCodeAt(++I) === QUOTE2
        );
        if ((i = I) >= N) eof = true;
        else if ((c3 = text.charCodeAt(I++)) === NEWLINE2) eol = true;
        else if (c3 === RETURN2) {
          eol = true;
          if (text.charCodeAt(I) === NEWLINE2) ++I;
        }
        return text.slice(j + 1, i - 1).replace(/""/g, '"');
      }
      while (I < N) {
        if ((c3 = text.charCodeAt((i = I++))) === NEWLINE2) eol = true;
        else if (c3 === RETURN2) {
          eol = true;
          if (text.charCodeAt(I) === NEWLINE2) ++I;
        } else if (c3 !== DELIMITER) continue;
...
function addAll(data) {
  ...
  for (i = 0; i < n; ++i) {
    if (
      isNaN((x3 = +this._x.call(null, (d = data[i])))) ||
      isNaN((y3 = +this._y.call(null, d)))
    )
      continue;
    xz[i] = x3;
    yz[i] = y3;
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (
      ...
      sum7.add(
        atan22(
          k2 * sign3 * sin3(absDelta),
          cosPhi0 * cosPhi1 + k2 * cos3(absDelta),
        ),
      );
      ...
      if (antimeridian ^ (lambda0 >= lambda) ^ (lambda1 >= lambda)) {
        ...
        var phiArc =
          (antimeridian ^ (delta >= 0) ? -1 : 1) * asin2(intersection5[2]);
  ...
  return (
    (angle < -epsilon7 || (angle < epsilon7 && sum7 < -epsilon22)) ^
    (winding & 1)
  );
}
  ...
  return {
    ...
    point: function (lambda1, phi1) {
      ...
      if (abs4(delta - pi5) < epsilon7) {
        stream.point(
          lambda0,
          (phi0 = (phi0 + phi1) / 2 > 0 ? halfPi4 : -halfPi4),
        );
...
var uniform_default = (function sourceRandomUniform(source) {
  function randomUniform(min7, max8) {
    ...
    if (arguments.length === 1) (max8 = min7), (min7 = 0);
    else max8 -= min7;
    return function () {
...
var int_default = (function sourceRandomInt(source) {
  function randomInt(min7, max8) {
    if (arguments.length < 2) (max8 = min7), (min7 = 0);
...
var irwinHall_default = (function sourceRandomIrwinHall(source) {
  function randomIrwinHall(n) {
    if ((n = +n) <= 0) return () => 0;
    return function () {
      for (var sum7 = 0, i = n; i > 1; --i) sum7 += source();
...
Transform.prototype = {
  ...
  rescaleX: function (x3) {
    return x3
      .copy()
      .domain(x3.range().map(this.invertX, this).map(x3.invert, x3));
  },
  rescaleY: function (y3) {
    return y3
      .copy()
      .domain(y3.range().map(this.invertY, this).map(y3.invert, y3));
  },
  toString: function () {
...
//# sourceMappingURL=chunk-OQUVF2X3.js.map
