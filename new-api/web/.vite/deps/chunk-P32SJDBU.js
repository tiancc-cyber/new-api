...
import {
  ...
  styles2String,
  userNodeOverrides,
} from './chunk-RK4ST7HX.js';
import {
  configureLabelImages,
  getSubGraphTitleMargins,
} from './chunk-GCN5RWLQ.js';
import { createText, getIconSVG } from './chunk-KS234T37.js';
import {
  calculateTextWidth,
  decodeEntities,
  handleUndefinedAttr,
...
var labelHelper = __name(async (parent, node, _classes) => {
  ...
  const useHtmlLabels =
    node.useHtmlLabels ||
    evaluate((_a = getConfig2()) == null ? void 0 : _a.htmlLabels);
  if (!_classes) {
  ...
  const text2 = await createText(
    ...
    sanitizeText(decodeEntities(label), getConfig2()),
    {
      ...
      width:
        node.width ||
        ((_b = getConfig2().flowchart) == null ? void 0 : _b.wrappingWidth),
...
var insertLabel = __name(async (parent, label, options) => {
  var _a, _b;
  const useHtmlLabels =
    options.useHtmlLabels ?? getEffectiveHtmlLabels(getConfig2());
...
var createLabel = __name(
  async (element, _vertexText, style, isTitle = false, isNode = false) => {
...
var rect = __name(async (parent, node) => {
  ...
  const { labelStyles, nodeStyles, borderStyles, backgroundStyles } =
    styles2String(node);
  const shapeSvg = parent
    .insert('g')
  ...
  if (node.labelType === 'markdown') {
    ...
    text2 = await createLabel_default(
      labelEl,
      node.label,
      node.labelStyle || '',
      false,
  ...
  if (node.look === 'handDrawn') {
    ...
    const roughNode = rc.path(
      createRoundedRectPathD(x, y, width, height, 0),
      options,
    );
...
var roundedWithTitle = __name(async (parent, node) => {
  ...
  const text2 = await createLabel_default(
    label,
    node.label,
    node.labelStyle,
    void 0,
...
var kanbanSection = __name(async (parent, node) => {
  ...
  if (node.look === 'handDrawn') {
    ...
    const roughNode = rc.path(
      createRoundedRectPathD(x, y, width, height, node.rx),
      options,
    );
    rect2 = shapeSvg.insert(() => {
      log.debug('Rough node insert CXC', roughNode);
...
function generateArcPoints(x1, y1, x2, y2, rx, ry, clockwise) {
  ...
  const centerX =
    midX + scaledCenterDistance * ry * Math.sin(angle) * (clockwise ? -1 : 1);
  const centerY =
    midY - scaledCenterDistance * rx * Math.cos(angle) * (clockwise ? -1 : 1);
...
async function bowTieRect(parent, node) {
  ...
  const bowTieRectShape = shapeSvg.insert(
    () => bowTieRectShapePath,
    ':first-child',
  );
...
async function curvedTrapezoid(parent, node) {
  ...
  const w = Math.max(
    minWidth,
    (bbox.width + (node.padding ?? 0) * 2) * 1.25,
...
async function cylinder(parent, node) {
  ...
  if (node.look === 'handDrawn') {
    ...
    const innerLine = rc.path(
      innerPathData,
      userNodeOverrides(node, { fill: 'none' }),
    );
...
function forkJoin(
  parent,
  node,
  { dir, config: { state: state2, themeVariables } },
) {
  const { nodeStyles } = styles2String(node);
  node.label = '';
  const shapeSvg = parent
...
async function iconRounded(
  ...
  const fill = stylesMap.get('fill');
  options.stroke = fill ?? mainBkg;
  const iconNode = rc.path(
    createRoundedRectPathD(x, y, width, height, 5),
    options,
  );
  const outerWidth = Math.max(width, bbox.width);
  const outerHeight = height + bbox.height + labelPadding;
  ...
  const iconShape = shapeSvg
    .insert(() => iconNode, ':first-child')
...
async function iconSquare(
  ...
  options.stroke = fill ?? mainBkg;
  const iconNode = rc.path(
    createRoundedRectPathD(x, y, width, height, 0.1),
    options,
  );
  const outerWidth = Math.max(width, bbox.width);
  const outerHeight = height + bbox.height + labelPadding;
...
async function imageSquare(parent, node, { config: { flowchart } }) {
  ...
  await img.decode();
  const imageNaturalWidth = Number(
    img.naturalWidth.toString().replace('px', ''),
  );
  const imageNaturalHeight = Number(
    img.naturalHeight.toString().replace('px', ''),
  );
...
async function drawRect(parent, node, options) {
  ...
  if (node.look === 'handDrawn') {
    ...
    const roughNode =
      rx || ry
        ? rc.path(
            createRoundedRectPathD(x, y, totalWidth, totalHeight, rx || 0),
            options2,
          )
...
var createCylinderPathD2 = __name(
  (x, y, width, height, rx, ry, outerOffset) => {
    return [
      `M${x},${y + ry}`,
      `a${rx},${ry} 0,0,0 ${width},0`,
      `a${rx},${ry} 0,0,0 ${-width},0`,
...
var createOuterCylinderPathD2 = __name(
  (x, y, width, height, rx, ry, outerOffset) => {
    return [
      `M${x},${y + ry}`,
      `M${x + width},${y + ry}`,
...
async function linedCylinder(parent, node) {
  ...
  if (node.look === 'handDrawn') {
    const rc = at.svg(shapeSvg);
    const outerPathData = createOuterCylinderPathD2(
      0,
      0,
      w,
      h,
...
async function multiWaveEdgedRectangle(parent, node) {
  ...
  const lastWavePoint =
    wavePoints == null ? void 0 : wavePoints[wavePoints.length - 1];
  const outerPathPoints = [
    { x: x - rectOffset, y: y + rectOffset },
...
async function note(parent, node, { config: { themeVariables } }) {
  ...
  node.labelStyle = labelStyles;
  const useHtmlLabels =
    node.useHtmlLabels || getEffectiveHtmlLabels(getConfig());
  if (!useHtmlLabels) {
...
async function squareRect2(parent, node) {
  const options = {
    ...
    classes: '',
    labelPaddingX:
      node.labelPaddingX ?? ((node == null ? void 0 : node.padding) || 0) * 2,
    labelPaddingY: ((node == null ? void 0 : node.padding) || 0) * 1,
  };
  return drawRect(parent, node, options);
}
...
async function taggedWaveEdgedRectangle(parent, node) {
  ...
  const waveEdgeRect = shapeSvg.insert(
    () => taggedWaveEdgeRectNode,
    ':first-child',
  );
...
async function triangle(parent, node) {
  ...
  const useHtmlLabels =
    node.useHtmlLabels || getEffectiveHtmlLabels(getConfig2());
  const w = bbox.width + (node.padding ?? 0);
  const h = w + bbox.height;
  const tw = w + bbox.height;
  const points = [
...
async function windowPane(parent, node) {
  ...
  windowPane2.attr(
    'transform',
    `translate(${rectOffset / 2}, ${rectOffset / 2})`,
  );
...
async function erBox(parent, node) {
  ...
  let TEXT_PADDING =
    ((_b = config.er) == null ? void 0 : _b.entityPadding) ?? 6;
  const { cssStyles } = node;
  const { labelStyles, nodeStyles } = styles2String(node);
  ...
  if (
    nameBBox.width +
      PADDING * 2 -
      (maxTypeWidth + maxNameWidth + maxKeysWidth + maxCommentWidth) >
    0
  ) {
    const difference =
      nameBBox.width +
  ...
  shapeSvg
    ...
    .attr(
      'transform',
      'translate(' + -nameBBox.width / 2 + ', ' + (y + TEXT_PADDING / 2) + ')',
    );
...
async function textHelper(
  ...
  if (annotationGroup !== null) {
    ...
    annotationGroup.attr(
      'transform',
      `translate(${-annotationGroupBBox.width / 2})`,
    );
  }
  labelGroup.attr(
    'transform',
    `translate(${-labelGroupBBox.width / 2}, ${annotationGroupHeight})`,
  );
...
async function addText2(parentGroup, node, yOffset, styles = []) {
  const textEl = parentGroup
    .insert('g')
    .attr('class', 'label')
    .attr('style', styles.join('; '));
  const config = getConfig();
  let useHtmlLabels =
    'useHtmlLabels' in node
      ? node.useHtmlLabels
      : (evaluate(config.htmlLabels) ?? true);
  let textContent = '';
  if ('text' in node) {
  ...
  if (!useHtmlLabels) {
    ...
    if (text2.textContent === '' || text2.textContent.includes('&gt')) {
      ...
      if (preserveSpace) {
        textChild.textContent =
          textChild.textContent[0] + ' ' + textChild.textContent.substring(1);
      }
    }
            ...
            new Promise((res) => {
              function setupImage() {
                ...
                if (noImgText) {
                  ...
                  const width =
                    parseInt(bodyFontSize, 10) * enlargingFactor + 'px';
  ...
  textEl.attr(
    'transform',
    'translate(0,' + (-bbox.height / (2 * numberOfLines) + yOffset) + ')',
  );
  return bbox.height;
}
...
async function classBox(parent, node) {
  ...
  const GAP = PADDING;
  const useHtmlLabels =
    node.useHtmlLabels ?? evaluate(config.htmlLabels) ?? true;
  const classNode = node;
  classNode.annotations = classNode.annotations ?? [];
  classNode.members = classNode.members ?? [];
  classNode.methods = classNode.methods ?? [];
  const { shapeSvg, bbox } = await textHelper(
    parent,
    node,
    config,
    useHtmlLabels,
  ...
  const styles =
    ((_a = classNode.styles) == null ? void 0 : _a.join(';')) ||
    nodeStyles ||
    '';
  if (!node.cssStyles) {
  ...
  shapeSvg.selectAll('.text').each((_, i, nodes) => {
    ...
    if (
      ...
      newTranslateX =
        -((_a2 = text2.node()) == null ? void 0 : _a2.getBBox().width) / 2 || 0;
...
async function requirementBox(parent, node) {
  ...
  const totalWidth =
    (((_a = shapeSvg.node()) == null ? void 0 : _a.getBBox().width) ?? 200) +
    padding;
  const totalHeight =
    (((_b = shapeSvg.node()) == null ? void 0 : _b.getBBox().height) ?? 200) +
    padding;
  const x = -totalWidth / 2;
  const y = -totalHeight / 2;
  const rc = at.svg(shapeSvg);
  ...
  shapeSvg.selectAll('.label').each((_, i, nodes) => {
    ...
    text2.attr(
      'transform',
      `translate(${newTranslateX}, ${newTranslateY + padding})`,
    );
  });
...
async function addText3(parentGroup, inputText, yOffset, style = '') {
  ...
  textEl.attr(
    'transform',
    `translate(${-bbox.width / 2},${-bbox.height / 2 + yOffset})`,
  );
  return bbox.height;
}
...
async function kanbanItem(parent, kanbanNode, { config }) {
  ...
  if (
    ...
    kanbanNode.ticket &&
    ((_a = config == null ? void 0 : config.kanban) == null
      ? void 0
      : _a.ticketBaseUrl)
  ) {
    ticketUrl =
  ...
  if (kanbanNode.look === 'handDrawn') {
    ...
    rect2
      .attr('class', 'basic label-container')
      .attr('style', cssStyles ? cssStyles : null);
  } else {
    rect2 = shapeSvg.insert('rect', ':first-child');
    rect2
...
//# sourceMappingURL=chunk-P32SJDBU.js.map
