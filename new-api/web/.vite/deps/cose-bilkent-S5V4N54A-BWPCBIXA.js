...
var import_cytoscape_cose_bilkent = __toESM(
...
async function executeCoseBilkentLayout(data, _config) {
  ...
  try {
    ...
    log.debug(
      `Layout completed: ${positionedNodes.length} nodes, ${positionedEdges.length} edges`,
    );
    return {
    ...
    await Promise.all(
      data4Layout.nodes.map(async (node) => {
        if (node.isGroup) {
          ...
          log2.debug(
            `Node ${node.id} dimensions: ${boundingBox.width}x${boundingBox.height}`,
          );
        }
      }),
    );
    ...
    await Promise.all(
      data4Layout.edges.map(async (edge) => {
        ...
        if (startNode && endNode) {
          const positionedEdge = layoutResult.edges.find(
            (e) => e.id === edge.id,
          );
          if (positionedEdge) {
...
//# sourceMappingURL=cose-bilkent-S5V4N54A-BWPCBIXA.js.map
