...
import { isLabelStyle, styles2String } from './chunk-RK4ST7HX.js';
...
var TreeMapDB =
  ((_a = class {
    ...
    getStylesForClass(classSelector) {
      var _a2;
      return (
        ((_a2 = this.classes.get(classSelector)) == null
          ? void 0
          : _a2.styles) ?? []
      );
    }
...
var populate = __name((ast, db) => {
  ...
  for (const row of ast.TreemapRows ?? []) {
    ...
    const styles = item.classSelector
      ? db.getStylesForClass(item.classSelector)
      : [];
...
var draw = __name((_text, id, _version, diagram2) => {
  ...
  const width = config.nodeWidth
    ? config.nodeWidth * SECTION_INNER_PADDING
    : 960;
  const height = config.nodeHeight
    ? config.nodeHeight * SECTION_INNER_PADDING
    : 500;
  ...
  try {
    ...
    if (formatStr === '$0,0') {
      ...
      valueFormat = __name(
        (value) => '$' + format(',' + precisionStr)(value),
        'valueFormat',
      );
    } else if (formatStr.startsWith('$')) {
      const restOfFormat = formatStr.substring(1);
      valueFormat = __name(
        (value) => '$' + format(restOfFormat || '')(value),
        'valueFormat',
      );
    } else {
  ...
  const treemapLayout = treemap_default()
    ...
    .paddingLeft((d) =>
      d.children && d.children.length > 0 ? SECTION_INNER_PADDING : 0,
    )
    .paddingRight((d) =>
      d.children && d.children.length > 0 ? SECTION_INNER_PADDING : 0,
    )
    ...
    .each(function (d) {
      ...
      if (config.showValues !== false && d.value) {
        ...
        const labelMustEndBeforeX =
          valueEndsAtXRelative -
          estimatedValueTextActualWidth -
          gapBetweenLabelAndValue;
        spaceForTextContent = labelMustEndBeforeX - labelXPosition;
      } else {
        const labelOwnRightPadding = 6;
        spaceForTextContent =
          totalHeaderWidth - labelXPosition - labelOwnRightPadding;
      }
      const minimumWidthToDisplay = 15;
      const actualAvailableWidth = Math.max(
        minimumWidthToDisplay,
        spaceForTextContent,
      );
      ...
      if (currentTextContentLength > actualAvailableWidth) {
        ...
        while (currentTruncatedText.length > 0) {
          currentTruncatedText = originalText.substring(
            0,
            currentTruncatedText.length - 1,
          );
  ...
  leafLabels.each(function (d) {
    ...
    let prospectiveValueFontSize = Math.max(
      minValueFontSize,
      Math.min(
        originalValueRelFontSize,
        Math.round(currentLabelFontSize * valueScaleFactor),
      ),
    );
    let combinedHeight =
      currentLabelFontSize +
      spacingBetweenLabelAndValue +
      prospectiveValueFontSize;
    while (
      combinedHeight > availableHeight &&
      currentLabelFontSize > minLabelFontSize
    ) {
      currentLabelFontSize--;
  ...
  if (config.showValues !== false) {
    ...
    leafValues.each(function (d) {
      ...
      const labelElement =
        select_default(parentCellNode).select('.treemapLabel');
      ...
      const actualValueFontSize = Math.max(
        ...
        Math.min(
          ...
          Math.round(finalLabelFontSize * valueScaleFactor),
        ),
      );
      valueTextElement.style('font-size', `${actualValueFontSize}px`);
      const labelCenterY = (d.y1 - d.y0) / 2;
      const valueTopActualY =
        labelCenterY + finalLabelFontSize / 2 + spacingBetweenLabelAndValue;
      ...
      if (
        valueTextElement.node().getComputedTextLength() >
...
export { diagram };
//# sourceMappingURL=diagram-E7M64L7V-LVXVMMJ5.js.map
