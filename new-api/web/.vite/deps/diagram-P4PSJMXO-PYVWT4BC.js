...
var populate = __name((ast, db) => {
  ...
  for (let { start, end, bits, label } of ast.blocks) {
    ...
    while (
      word.length <= bitsPerRow + 1 &&
      db.getPacket().length < maxPacketSize
    ) {
...
var draw = __name((_text, id, _version, diagram2) => {
  ...
  const svgHeight =
    totalRowHeight * (words.length + 1) - (title ? 0 : rowHeight);
...
export { diagram };
//# sourceMappingURL=diagram-P4PSJMXO-PYVWT4BC.js.map
