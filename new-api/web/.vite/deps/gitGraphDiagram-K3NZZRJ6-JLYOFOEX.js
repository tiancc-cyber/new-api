...
import { cleanAndMerge, random, utils_default } from './chunk-IPOIRDR2.js';
...
var commit = __name(function (commitDB) {
  ...
  tags =
    tags == null
      ? void 0
      : tags.map((tag) => common_default.sanitizeText(tag, config));
  const newCommit = {
...
var branch = __name(function (branchDB) {
  ...
  state.records.branches.set(
    name,
    state.records.head != null ? state.records.head.id : null,
  );
...
var merge = __name((mergeDB) => {
  ...
  const currentBranchCheck = state.records.branches.get(
  ...
  const currentCommit = currentBranchCheck
    ...
    : void 0;
  const otherCommit = otherBranchCheck
  ...
  const commit2 = {
    ...
    seq: state.records.seq++,
    parents:
      state.records.head == null ? [] : [state.records.head.id, verifiedBranch],
    branch: state.records.currBranch,
...
var cherryPick = __name(function (cherryPickDB) {
  ...
  if (!targetId || !state.records.commits.has(targetId)) {
    ...
    const currentCommitId = state.records.branches.get(
      state.records.currBranch,
    );
    ...
    const commit2 = {
      ...
      parents:
        state.records.head == null
          ? []
          : [state.records.head.id, sourceCommit.id],
      branch: state.records.currBranch,
...
var parseStatement = __name((statement, db2) => {
  const parsers = {
    ...
    CherryPicking: __name(
      (stmt) => db2.cherryPick(parseCherryPicking(stmt)),
      'CherryPicking',
    ),
  };
...
var parseCommit = __name((commit2) => {
  const commitDB = {
    ...
    msg: commit2.message ?? '',
    type:
      commit2.type !== void 0 ? commitType[commit2.type] : commitType.NORMAL,
...
var parseCherryPicking = __name((cherryPicking) => {
  ...
  const cherryPickDB = {
    ...
    targetId: '',
    tags:
      ((_a = cherryPicking.tags) == null ? void 0 : _a.length) === 0
        ? void 0
        : cherryPicking.tags,
    ...
    const parentPosition =
      dir === 'TB' || dir == 'BT'
        ...
        : (_b = commitPos.get(parent)) == null
          ...
          : _b.x;
    if (
      parentPosition !== void 0 &&
      comparisonFunc(parentPosition, targetPosition)
    ) {
...
var findClosestParentPos = __name((commit2) => {
  ...
  const closestParent = findClosestParent(
    commit2.parents.filter((p) => p !== null),
  );
  if (!closestParent) {
    throw new Error(`Closest parent not found for commit ${commit2.id}`);
  }
  const closestParentPos =
    (_a = commitPos.get(closestParent)) == null ? void 0 : _a.y;
  if (closestParentPos === void 0) {
    throw new Error(
      `Closest parent position not found for commit ${commit2.id}`,
    );
  }
...
var drawCommitBullet = __name(
  (
    ...
    if (commitSymbolType === commitType.HIGHLIGHT) {
      ...
      circle.attr(
        'class',
        `commit ${commit2.id} commit${branchIndex % THEME_COLOR_LIMIT}`,
      );
...
var drawCommitLabel = __name(
  (gLabels, commit2, commitPosition, pos, gitGraphConfig) => {
    var _a;
    if (
...
var calculatePosition = __name((commit2, dir2, pos, commitPos2) => {
  ...
  if (commit2.parents.length > 0) {
    ...
    if (closestParent) {
      const parentPosition =
        commitPos2.get(closestParent) ?? defaultCommitPosition;
      if (dir2 === 'TB') {
        return parentPosition.y + COMMIT_STEP;
      } else if (dir2 === 'BT') {
        const currentPosition =
          commitPos2.get(commit2.id) ?? defaultCommitPosition;
...
var getCommitPosition = __name((commit2, pos, isParallelCommits) => {
  var _a, _b;
  const posWithOffset =
    dir === 'BT' && isParallelCommits ? pos : pos + LAYOUT_OFFSET;
  const y =
    dir === 'TB' || dir === 'BT'
      ? posWithOffset
      : (_a = branchPos.get(commit2.branch)) == null
        ? void 0
        : _a.pos;
  const x =
    dir === 'TB' || dir === 'BT'
...
var drawCommits = __name((svg, commits, modifyGraph, gitGraphConfig) => {
  ...
  sortedKeys.forEach((key) => {
    ...
    if (modifyGraph) {
      ...
      const commitSymbolType = commit2.customType ?? commit2.type;
      const branchIndex =
        ((_a = branchPos.get(commit2.branch)) == null ? void 0 : _a.index) ?? 0;
      drawCommitBullet(
        gBullets,
        commit2,
        commitPosition,
...
var shouldRerouteArrow = __name((commitA, commitB, p1, p2, allCommits) => {
  const commitBIsFurthest =
    dir === 'TB' || dir === 'BT' ? p1.x < p2.x : p1.y < p2.y;
  const branchToGetCurve = commitBIsFurthest ? commitB.branch : commitA.branch;
  const isOnBranchToGetCurve = __name(
    (x) => x.branch === branchToGetCurve,
...
var drawArrow = __name((svg, commitA, commitB, allCommits) => {
  ...
  const arrowNeedsRerouting = shouldRerouteArrow(
    commitA,
    commitB,
    p1,
    p2,
  ...
  let offset = 0;
  let colorClassNum =
    (_a = branchPos.get(commitB.branch)) == null ? void 0 : _a.index;
  if (commitB.type === commitType.MERGE && commitA.id !== commitB.parents[0]) {
    colorClassNum =
      (_b = branchPos.get(commitA.branch)) == null ? void 0 : _b.index;
  }
  let lineDef;
  if (arrowNeedsRerouting) {
    ...
    if (dir === 'TB') {
      if (p1.x < p2.x) {
        ...
        colorClassNum =
          (_c = branchPos.get(commitA.branch)) == null ? void 0 : _c.index;
      ...
      if (p1.x < p2.x) {
        ...
        colorClassNum =
          (_d = branchPos.get(commitA.branch)) == null ? void 0 : _d.index;
        lineDef = `M ${p1.x} ${p1.y} L ${lineX + radius} ${p1.y} ${arc2} ${lineX} ${p1.y - offset} L ${lineX} ${p2.y + radius} ${arc} ${lineX - offset} ${p2.y} L ${p2.x} ${p2.y}`;
      }
    } else {
      if (p1.y < p2.y) {
        ...
        colorClassNum =
          (_e = branchPos.get(commitA.branch)) == null ? void 0 : _e.index;
    ...
    if (dir === 'TB') {
      if (p1.x < p2.x) {
        if (
          commitB.type === commitType.MERGE &&
          commitA.id !== commitB.parents[0]
        ) {
...
var drawBranches = __name((svg, branches, gitGraphConfig) => {
  ...
  branches.forEach((branch2, index) => {
    ...
    bkg
      ...
      .attr(
        'x',
        -bbox.width - 4 - (gitGraphConfig.rotateCommitLabel === true ? 30 : 0),
      )
    ...
    if (dir === 'TB') {
      ...
      label.attr(
        'transform',
        'translate(' + (pos - bbox.width / 2 - 5) + ', 0)',
      );
    } else if (dir === 'BT') {
      bkg.attr('x', pos - bbox.width / 2 - 10).attr('y', maxPos);
      label.attr(
        'transform',
        'translate(' + (pos - bbox.width / 2 - 5) + ', ' + maxPos + ')',
      );
    } else {
...
var setBranchPosition = __name(function (
  name,
  pos,
  index,
  bbox,
...
export { diagram };
//# sourceMappingURL=gitGraphDiagram-K3NZZRJ6-JLYOFOEX.js.map
