...
import { __name, arc_default, select_default } from './chunk-OQUVF2X3.js';
    ...
    if (fullTextWidth <= maxLabelWidth) {
      ...
      words.forEach((word) => {
        ...
        if (textWidth > maxLabelWidth) {
          ...
          if (
            measureText.node().getBoundingClientRect().width > maxLabelWidth
          ) {
            let brokenWord = '';
...
var draw = __name(function (text, id, version, diagObj) {
  ...
  diagram2.attr(
    'viewBox',
    `${box.startx} -25 ${width} ${height + extraVertForTitle}`,
  );
    ...
    function updateFn(type) {
      return __name(function updateItemBounds(item) {
        ...
        _self.updateVal(
          bounds.data,
          'startx',
          startx - n * conf2.boxMargin,
          Math.min,
        );
        _self.updateVal(
          bounds.data,
          'stopx',
          stopx + n * conf2.boxMargin,
          Math.max,
        );
        if (!(type === 'activation')) {
          _self.updateVal(
          ...
          _self.updateVal(
            bounds.data,
            'starty',
            starty - n * conf2.boxMargin,
            Math.min,
          );
          _self.updateVal(
            bounds.data,
            'stopy',
            stopy + n * conf2.boxMargin,
            Math.max,
          );
        }
      }, 'updateItemBounds');
...
var drawTasks = __name(function (diagram2, tasks2, verticalPos) {
  ...
  for (const [i, task] of tasks2.entries()) {
    ...
    bounds.insert(
      task.x,
      task.y,
      task.x + task.width + conf2.taskMargin,
      300 + 5 * 30,
    );
  }
}, 'drawTasks');
...
export { diagram };
//# sourceMappingURL=journeyDiagram-4ABVD52K-5XDKLTJE.js.map
