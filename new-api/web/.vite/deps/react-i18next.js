...
function getChildren(node) {
  ...
  return node.props && node.props.i18nIsDynamicList
    ? getAsArray(children)
    : children;
}
...
function renderNodes(
  ...
  getData(children);
  const ast = html_parse_stringify_module_default.parse(
    `<0>${targetString}</0>`,
  );
  const opts = {
    ...data,
  ...
  (0, import_react4.useEffect)(() => {
    ...
    return () => {
      ...
      if (bindI18nStore && i18n)
        bindI18nStore
          .split(' ')
          .forEach((e2) => i18n.store.off(e2, boundReset));
    };
  }, [i18n, joinedNS]);
