...
function max_default(values, valueof) {
  ...
  if (valueof == null) {
    ...
    while (++i < n) {
      if (
        (value2 = valueof(values[i], i, values)) != null &&
        value2 >= value2
      ) {
        max2 = value2;
        while (++i < n) {
...
function center(node) {
  return node.targetLinks.length
    ...
    : node.sourceLinks.length
      ? min_default(node.sourceLinks, targetDepth) - 1
      : 0;
}
...
function Sankey() {
  ...
  function computeNodeLayers({ nodes: nodes3 }) {
    ...
    for (const node of nodes3) {
      const i = Math.max(
        0,
        Math.min(x2 - 1, Math.floor(align.call(null, node, x2))),
      );
      node.layer = i;
      node.x0 = x0 + i * kx2;
      node.x1 = node.x0 + dx;
  ...
  function initializeNodeBreadths(columns) {
    const ky2 = min_default(
      columns,
      (c) => (y1 - y0 - (c.length - 1) * py) / sum_default(c, value),
    );
...
Path.prototype = path.prototype = {
  ...
  moveTo: function (x2, y2) {
    this._ +=
      'M' + (this._x0 = this._x1 = +x2) + ',' + (this._y0 = this._y1 = +y2);
  },
  closePath: function () {
    if (this._x1 !== null) {
  ...
  quadraticCurveTo: function (x1, y1, x2, y2) {
    this._ +=
      'Q' + +x1 + ',' + +y1 + ',' + (this._x1 = +x2) + ',' + (this._y1 = +y2);
  },
...
var parser = (function () {
  ...
  var parser2 = {
    ...
    performAction: __name(function anonymous(
      ...
      switch (yystate) {
        case 7:
          const source = yy.findOrCreateNode(
            $$[$0 - 4].trim().replaceAll('""', '"'),
          );
          const target = yy.findOrCreateNode(
            $$[$0 - 2].trim().replaceAll('""', '"'),
          );
...
var draw = __name(function (text, id, _version, diagObj) {
  ...
  const width =
    (conf == null ? void 0 : conf.width) ?? defaultSankeyConfig.width;
  const height =
    (conf == null ? void 0 : conf.height) ?? defaultSankeyConfig.width;
  const useMaxWidth =
    (conf == null ? void 0 : conf.useMaxWidth) ??
    defaultSankeyConfig.useMaxWidth;
  const nodeAlignment =
    (conf == null ? void 0 : conf.nodeAlignment) ??
    defaultSankeyConfig.nodeAlignment;
  const prefix =
    (conf == null ? void 0 : conf.prefix) ?? defaultSankeyConfig.prefix;
  const suffix =
    (conf == null ? void 0 : conf.suffix) ?? defaultSankeyConfig.suffix;
  const showValues =
    (conf == null ? void 0 : conf.showValues) ?? defaultSankeyConfig.showValues;
...
export { diagram };
//# sourceMappingURL=sankeyDiagram-WA2Y5GQK-ST64I4JU.js.map
