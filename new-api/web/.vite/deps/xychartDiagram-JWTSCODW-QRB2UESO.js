...
import { computeDimensionOfText } from './chunk-KS234T37.js';
...
import { __name, band, line_default, linear, log } from './chunk-OQUVF2X3.js';
  ...
  ((_a = class {
    ...
    getMaxDimension(texts, fontSize) {
      if (!this.parentGroup) {
        return {
          width:
            texts.reduce((acc, cur) => Math.max(cur.length, acc), 0) * fontSize,
          height: fontSize,
        };
      }
      const dimension = {
...
var BaseAxis =
  ((_a2 = class {
    ...
    recalculateOuterPaddingToDrawBar() {
      if (
        BAR_WIDTH_TO_TICK_WIDTH_RATIO * this.getTickDistance() >
        this.outerPadding * 2
      ) {
        this.outerPadding = Math.floor(
          (BAR_WIDTH_TO_TICK_WIDTH_RATIO * this.getTickDistance()) / 2,
        );
      }
      this.recalculateScale();
    }
    calculateSpaceIfDrawnHorizontally(availableSpace) {
      ...
      if (this.axisConfig.showLabel) {
        ...
        const maxPadding =
          MAX_OUTER_PADDING_PERCENT_FOR_WRT_LABEL * availableSpace.width;
        this.outerPadding = Math.min(spaceRequired.width / 2, maxPadding);
        const heightRequired =
          spaceRequired.height + this.axisConfig.labelPadding * 2;
      ...
      if (this.axisConfig.showTitle && this.title) {
        ...
        const heightRequired =
          spaceRequired.height + this.axisConfig.titlePadding * 2;
    ...
    calculateSpaceIfDrawnVertical(availableSpace) {
      ...
      if (this.axisConfig.showLabel) {
        const spaceRequired = this.getLabelDimension();
        const maxPadding =
          MAX_OUTER_PADDING_PERCENT_FOR_WRT_LABEL * availableSpace.height;
        this.outerPadding = Math.min(spaceRequired.height / 2, maxPadding);
        const widthRequired =
          spaceRequired.width + this.axisConfig.labelPadding * 2;
...
function getAxis(data, axisConfig, axisThemeConfig, tmpSVGGroup2) {
  const textDimensionCalculator = new TextDimensionCalculatorWithFont(
    tmpSVGGroup2,
  );
...
var ChartTitle =
  ((_a5 = class {
    ...
    calculateSpace(availableSpace) {
      ...
      const widthRequired = Math.max(
        titleDimension.width,
        availableSpace.width,
      );
      const heightRequired =
        titleDimension.height + 2 * this.chartConfig.titlePadding;
      if (
  ...
  _a5);
function getChartTitleComponent(
  chartConfig,
  chartData,
  chartThemeConfig,
  tmpSVGGroup2,
...
var plotColorPalette = xyChartThemeConfig.plotColorPalette
  .split(',')
...
function setYAxisRangeFromPlotData(data) {
  ...
  const prevMinValue = isLinearAxisData(xyChartData.yAxis)
    ? xyChartData.yAxis.min
    : Infinity;
  const prevMaxValue = isLinearAxisData(xyChartData.yAxis)
    ? xyChartData.yAxis.max
    : -Infinity;
  xyChartData.yAxis = {
    type: 'linear',
    title: xyChartData.yAxis.title,
...
function transformDataWithoutCategory(data) {
  ...
  if (!hasSetXAxis) {
    const prevMinValue = isLinearAxisData(xyChartData.xAxis)
      ? xyChartData.xAxis.min
      : Infinity;
    const prevMaxValue = isLinearAxisData(xyChartData.xAxis)
      ? xyChartData.xAxis.max
      : -Infinity;
    setXAxisRangeData(
      Math.min(prevMinValue, 1),
      Math.max(prevMaxValue, data.length),
    );
  }
  if (!hasSetYAxis) {
...
function getPlotColorFromPalette(plotIndex2) {
  return plotColorPalette[
    plotIndex2 === 0 ? 0 : plotIndex2 % plotColorPalette.length
  ];
}
          ...
          if (chartConfig.chartOrientation === 'horizontal') {
            ...
            let fitsInBar2 = function (item, fontSize, yOffset2) {
              ...
              const horizontalFits =
                leftEdge >= data.x && rightEdge <= data.x + data.width;
              const verticalFits =
                data.y + yOffset2 + fontSize <= data.y + data.height;
      ...
      case 'text':
        shapeGroup
          ...
          .attr('dominant-baseline', (data) =>
            getDominantBaseLine(data.verticalPos),
          )
...
export { diagram };
//# sourceMappingURL=xychartDiagram-JWTSCODW-QRB2UESO.js.map
