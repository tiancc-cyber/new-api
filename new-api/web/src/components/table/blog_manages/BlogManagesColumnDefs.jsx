...
const BlogOperate = ({ record, openEdit, setBlogStatus, removeBlog, t }) => {
  ...
  const toggleStatus = () => {
    if (isPublished) {
      Modal.confirm({
        title: t('确认设为草稿'),
        content: t('设为草稿后，将不会对外展示（如果前台接入）。是否继续？'),
        centered: true,
        onOk: () => setBlogStatus(record, 0),
      });
    } else {
      Modal.confirm({
        title: t('确认发布'),
        content: t('发布后文章将被标记为已发布。是否继续？'),
        centered: true,
        onOk: () => setBlogStatus(record, 1),
      });
    }
  };
