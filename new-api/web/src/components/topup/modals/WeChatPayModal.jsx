...
export default function WeChatPayModal({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // eslint-disable-next-line no-console
      console.debug(
        'WeChatPayModal: render QR, codeUrl length=',
        codeUrl.length,
      );
      QRCode.toCanvas(
        canvas,
        ...
        async (err) => {
          ...
          // eslint-disable-next-line no-console
          console.error(
            'WeChatPayModal: canvas QR render failed, fallback to SVG',
            err,
          );
          try {
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      Modal.confirm({
        ...
        content: queryOk
          ? t(
              '关闭后将视为取消支付，若你已在微信完成支付请先点击「我已支付」确认。',
            )
          : t(
              '当前无法确认支付状态。若直接关闭将视为取消支付，建议稍后重试查询或点击「我已支付」。',
            ),
        okText: t('确认关闭'),
        cancelText: t('继续支付'),
        onOk: () => {
          ...
          >
            ...
            <canvas
              ...
              style={{
                ...
                height: 220,
                display:
                  codeUrl && !qrLoading && !qrSvgDataUrl ? 'block' : 'none',
              }}
