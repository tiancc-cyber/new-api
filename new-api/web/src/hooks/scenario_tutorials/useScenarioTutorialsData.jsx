...
export const useScenarioTutorialsData = () => {
  ...
  const [compactMode, setCompactMode] =
    useTableCompactMode('scenario_tutorials');
    ...
    const run = async () => {
      ...
      try {
        const res = await API.get(
          `/api/scenario_tutorial/admin/tutorials/${row.id}`,
        );
        if (res?.data?.success) {
  ...
  const saveRow = async (payload) => {
    ...
    try {
      ...
      if (editingRow?.id) {
        res = await API.put(
          `/api/scenario_tutorial/admin/tutorials/${editingRow.id}`,
          payload,
        );
      } else {
  ...
  const setRowStatus = async (record, status) => {
    ...
    try {
      const res = await API.patch(
        `/api/scenario_tutorial/admin/tutorials/${record.id}/status`,
        {
          status,
        },
      );
  ...
  const removeRow = async (record) => {
    ...
    try {
      const res = await API.delete(
        `/api/scenario_tutorial/admin/tutorials/${record.id}`,
      );
      if (res.data?.success) {
        showSuccess(t('删除成功'));
        await loadRows();
