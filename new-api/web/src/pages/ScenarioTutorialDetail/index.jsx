...
import { Button, Card, Typography, Skeleton, Tag, Empty } from '@douyinfe/semi-ui';
...
const ScenarioTutorialDetail = () => {
  ...
  const load = async () => {
	setLoading(true);
	try {
	  const res = await API.get(`/api/scenario_tutorial/public/tutorials/md5/${md5}`);
	  if (res.data?.success) {
	...
	<PageContainer>
	  ...
	  <div
		style={{
		  ...
		  background: themeMode === 'dark' ? 'rgba(88,28,135,0.18)' : 'rgba(124,58,237,0.10)',
		  position: 'relative',
		  overflow: 'hidden',
		  padding: 14,
		  marginBottom: 12,
	  ...
	  >
		...
		<div
		  ...
		  style={{
			...
			inset: 0,
			pointerEvents: 'none',
			opacity: themeMode === 'dark' ? 0.10 : 0.08,
		...
		<div
		  ...
		  style={{
			...
			boxShadow:
			  themeMode === 'dark' ? '0 12px 26px rgba(0,0,0,0.28)' : '0 12px 24px rgba(17,24,39,0.08)',
		  }}
		>
			<div className='flex flex-col gap-2 min-w-0'>
			  <Button
				...
				style={{
				  ...
				  color:
					themeMode === 'dark' ? 'rgba(255,255,255,0.96)' : 'rgba(88,28,135,0.95)',
				  fontWeight: 800,
				  letterSpacing: 0.2,
				  boxShadow:
					themeMode === 'dark'
			  ...
			  <div className='min-w-0'>
				<Title
				  ...
				  style={{
					...
					textShadow: themeMode === 'dark' ? '0 1px 0 rgba(0,0,0,0.25)' : 'none',
				  }}
				>
				  {row?.title || ''}
				</Title>
